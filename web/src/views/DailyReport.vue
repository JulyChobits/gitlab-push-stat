  ...
  <div>
    ...
    <div style="display:flex;flex-direction:column;gap:16px">
        ...
        <el-table :data="reports" stripe v-loading="loading" border size="small" style="width:100%">
          ...
          <el-table-column prop="net_additions" width="95" align="right">
            ...
            <template #default="{ row }">
              <span :class="row.net_additions >= 0 ? 'text-green' : 'text-red'">
                {{ row.net_additions >= 0 ? '+' : '' }}{{ row.net_additions }}
              </span>
            </template>
          </el-table-column>
          <el-table-column prop="effective_lines" width="100" align="right">
            ...
            <template #default="{ row }">
              <span :class="row.effective_lines >= 0 ? 'text-green' : 'text-red'">
                {{ row.effective_lines >= 0 ? '+' : '' }}{{ row.effective_lines }}
              </span>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-card shadow="hover">
        ...
        <div style="margin-top:16px;display:flex;justify-content:flex-end">
          <el-pagination v-model:current-page="authorPage" :page-size="authorPageSize" :total="authorTotal"
            layout="total, prev, pager, next" @current-change="loadAuthorReports" />
        </div>
      </el-card>
    </div>

    <!-- 生成报表对话框 -->
    <el-dialog v-model="showGenDialog" title="生成日报表" width="400">
      <el-form label-width="80">
        <el-form-item label="日期">
          <el-date-picker v-model="genDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" style="width:100%" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { getDailyReports, generateDailyReport, getDailyAuthorSummary } from '../api'
...
const genDate = ref(dayjs().subtract(1, 'day').format('YYYY-MM-DD'))
...
const loadReports = async () => {
  ...
  try {
    ...
    const { data } = await getDailyReports(params)
...
const loadAuthorReports = async () => {
  ...
  try {
    ...
    const { data } = await getDailyAuthorSummary(params)
...
const doGenerate = async () => {
  if (!genDate.value) { ElMessage.warning('请选择日期'); return }
  generating.value = true
  try {
    await generateDailyReport(genDate.value)
