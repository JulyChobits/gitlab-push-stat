<template>
  <div>
    ...
    <div class="global-toolbar">
      <div class="toolbar-left">
        ...
        <el-date-picker v-model="dateRange" type="daterange" range-separator="~"
          ...
          size="default" @change="handleFilterChange" style="width:300px" />
      </div>
    ...
    <div style="display:flex;flex-direction:column;gap:16px">
      ...
      <el-card shadow="hover">
        ...
        <el-table :data="reports" stripe v-loading="loading" border size="small" style="width:100%">
          <el-table-column label="周期" width="190"><template #default="{ row }">{{ row.week_start }} ~ {{ row.week_end }}</template></el-table-column>
          ...
          <el-table-column prop="additions" width="95" align="right">
            <template #header><span>新增 <el-tooltip content="原始新增代码行数，包含所有文件（含自动生成、配置文件等）" placement="top" :show-after="100" popper-class="custom-tooltip"><el-icon class="tip-icon"><QuestionFilled /></el-icon></el-tooltip></span></template>
          ...
          <el-table-column prop="deletions" width="95" align="right">
            <template #header><span>减少 <el-tooltip content="原始删除代码行数，包含所有文件" placement="top" :show-after="100" popper-class="custom-tooltip"><el-icon class="tip-icon"><QuestionFilled /></el-icon></el-tooltip></span></template>
          ...
          <el-table-column prop="net_additions" width="95" align="right">
            <template #header><span>净增 <el-tooltip content="净增行数 = 新增行数 - 删除行数" placement="top" :show-after="100" popper-class="custom-tooltip"><el-icon class="tip-icon"><QuestionFilled /></el-icon></el-tooltip></span></template>
          ...
          <el-table-column prop="effective_lines" width="100" align="right">
            <template #header><span>有效行数 <el-tooltip content="过滤后的有效代码行数，排除了自动生成文件、配置文件、lock文件等噪音代码" placement="top" :show-after="100" popper-class="custom-tooltip"><el-icon class="tip-icon"><QuestionFilled /></el-icon></el-tooltip></span></template>
    ...
    <el-dialog v-model="showGenDialog" title="生成周报表" width="460">
      <el-form label-width="80">
        <el-form-item label="开始日期"><el-date-picker v-model="genWeekStart" type="date" placeholder="周一" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item>
        <el-form-item label="结束日期"><el-date-picker v-model="genWeekEnd" type="date" placeholder="周日" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item>
      </el-form>
      <template #footer>
...
import { getWeeklyReports, generateWeeklyReport, getWeeklyAuthorSummary } from '../api'
...
const genWeekStart = ref(now.subtract(weekday + 6, 'day').format('YYYY-MM-DD'))
const genWeekEnd = ref(now.subtract(weekday, 'day').format('YYYY-MM-DD'))
...
const loadReports = async () => {
  ...
  try {
    ...
    const { data } = await getWeeklyReports(params)
...
const loadAuthorReports = async () => {
  ...
  try {
    ...
    const { data } = await getWeeklyAuthorSummary(params)
...
const doGenerate = async () => {
  if (!genWeekStart.value || !genWeekEnd.value) { ElMessage.warning('请选择日期范围'); return }
  generating.value = true
  try {
    await generateWeeklyReport(genWeekStart.value, genWeekEnd.value)
    ElMessage.success('周报表生成成功')
